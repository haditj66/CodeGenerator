...
public class CMDHandlerVSDev : CMDHandler
{
    public CMDHandlerVSDev(string StartingWorkingDirtory, string DirectoryToPutBatchFile) : base(StartingWorkingDirtory)
    {
        ...
        //run that batch file 
        //string envVar = File.ReadAllText("EnironVariables.txt");

        ExecuteCommand(AppDomain.CurrentDomain.BaseDirectory+"\\GetVSEnironmentVariables.bat");

        //now get the environment variables that the batch file produced 
        using (StreamReader sr = new StreamReader(AppDomain.CurrentDomain.BaseDirectory + "\\EnironVariables.txt")) 
        {
            string line;
